...
Adam::Adam(std::vector<Tensor> parameters, const float learning_rate, const float beta1, const float beta2,
           const float epsilon)
...
void Adam::step() {
    ...
    for (std::size_t i = 0; i < trackedParameters.size(); ++i) {
        ...
        Tensor update = (firstMomentHat / denominator) * TensorImpl::full(gradient->get_shape(), learningRate);
